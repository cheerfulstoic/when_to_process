defmodule WhenToProcess.RidesTest do
  ...
  describe "list" do
    ...
    test "A driver" do
      {:ok, %{uuid: uuid}} = Rides.create(Rides.Driver, params_for(:driver))

      assert [%{uuid: ^uuid}] = Rides.list(Rides.Driver)
    end

    test "Two drivers" do
      {:ok, driver1} = Rides.create(Rides.Driver, params_for(:driver))
      {:ok, driver2} = Rides.create(Rides.Driver, params_for(:driver))
    ...
    test "Many drivers" do
      drivers =
        Enum.map(0..10, fn _ ->
          {:ok, driver} = Rides.create(Rides.Driver, params_for(:driver))
  ...
  describe "count" do
    ...
    test "A driver" do
      Rides.create(Rides.Driver, params_for(:driver))
    ...
    test "Many drivers" do
      Enum.map(0..10, fn _ -> Rides.create(Rides.Driver, params_for(:driver)) end)
  ...
  describe "set_position" do
    test "Can change latitude and longitude" do
      {:ok, driver} = Rides.create(Rides.Driver, params_for(:driver, latitude: 1.23, longitude: -3.21))
  ...
  describe "accept_ride_request" do
    test "ride is created" do
      {:ok, ride_request} = Rides.create(Rides.RideRequest, params_for(:ride_request))
      {:ok, driver} = Rides.create(Rides.Driver, params_for(:driver, ready_for_passengers: true))

      {:ok, %Rides.RideRequest{created_ride: created_ride}} = Rides.accept_ride_request(ride_request, driver)

      assert created_ride.driver_id == driver.id
    end

    test "cannot accept cancelled RideRequest" do
      {:ok, ride_request} = Rides.create(Rides.RideRequest, params_for(:ride_request, cancelled_at: DateTime.utc_now()))
      {:ok, driver} = Rides.create(Rides.Driver, params_for(:driver))
